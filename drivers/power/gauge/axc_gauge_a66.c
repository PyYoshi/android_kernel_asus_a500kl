...
#define DEFAULT_DEVICE_RESISTOR_VALUE_A600KL 100
#elif defined(ASUS_A500KL_PROJECT)
#define DEFAULT_DEVICE_RESISTOR_VALUE_A500KL 100
#endif
...
#ifdef ASUS_A600KL_PROJECT
int A600KL_SrcOCVtoPerTbl[OCV_TBL_SIZE]={
	3401, 3448, 3522, 3578, 3622, 3656, 3669, 3674, 3677, 3678,
	3679, 3680, 3681, 3684, 3691, 3699, 3704, 3710, 3715, 3721, 
	3726, 3730, 3733, 3736, 3739, 3743, 3747, 3750, 3753, 3757, 
	3760, 3764, 3768, 3772, 3775, 3777, 3781, 3784, 3786, 3789,
	3793, 3796, 3799, 3802, 3807, 3811, 3815, 3819, 3823, 3828, 

	3833, 3837, 3842, 3847, 3853, 3858, 3864, 3871, 3877, 3885, 
	3893, 3903, 3914, 3925, 3935, 3945, 3952, 3958, 3964, 3971, 
	3979, 3990, 4003, 4019, 4035, 4047, 4054, 4058, 4066, 4076, 
	4086, 4096, 4107, 4117, 4128, 4138, 4149, 4159, 4170, 4180, 
	4190, 4202, 4212, 4223, 4234, 4245, 4257, 4267, 4278, 4290, 
	4302
};
#endif
#ifdef ASUS_A500KL_PROJECT
int A500KL_SrcOCVtoPerTbl[OCV_TBL_SIZE]={
	3400, 3448, 3521, 3577, 3623, 3658, 3672, 3677, 3679, 3680,
	3681, 3683, 3684, 3687, 3695, 3703, 3708, 3713, 3720, 3725, 
	3731, 3734, 3737, 3740, 3744, 3748, 3752, 3756, 3760, 3763, 
	3768, 3771, 3775, 3778, 3782, 3784, 3788, 3791, 3795, 3798,
	3802, 3805, 3810, 3813, 3818, 3822, 3827, 3831, 3836, 3841, 

	3846, 3851, 3857, 3863, 3869, 3875, 3883, 3890, 3899, 3908, 
	3918, 3927, 3938, 3948, 3956, 3961, 3968, 3975, 3986, 3999, 
	4014, 4030, 4045, 4053, 4059, 4065, 4075, 4085, 4095, 4106, 
	4116, 4127, 4137, 4148, 4158, 4169, 4179, 4190, 4201, 4212, 
	4223, 4233, 4245, 4256, 4266, 4278, 4288, 4300, 4312, 4324, 
	4335
};
#endif
...
//ASUS_BSP Eason read battery ID+++  
	...
	this->resistorCali = DEFAULT_DEVICE_RESISTOR_VALUE_A600KL;
#elif defined(ASUS_A500KL_PROJECT)
	this->resistorCali = DEFAULT_DEVICE_RESISTOR_VALUE_A500KL;
#else
...
//ASUS_BSP Eason read battery ID+++    
#if defined(ASUS_A600KL_PROJECT)
	resistor = DEFAULT_DEVICE_RESISTOR_VALUE_A600KL;
#elif defined(ASUS_A500KL_PROJECT)
	resistor = DEFAULT_DEVICE_RESISTOR_VALUE_A500KL;
#else
	if(g_BatteryID_value<1100000){
...
{
#if defined(ASUS_A600KL_PROJECT)
	int Vf300TableSec[20] = {34,37,39,35,35,34,36,38,37,39,43,33,34,34,36,35,36,38, 40, 13};
							//3447 to 4340	interval:47 mV, total:20
#elif defined(ASUS_A500KL_PROJECT)
	int Vf300TableSec[20] = {43,45,48,46,43,43,46,45,46,49,47,43,42,43,47,44,45,47, 48, 50};
#else
...
{
#if defined(ASUS_A600KL_PROJECT)
	int Vf900TableSec[20] = {137,141,148,144,137,137,141,136,137,149,161,142,147,147,157,156,145,96,71, 23};
							//3447 to 4340	interval:47 mV, total:20
#elif defined(ASUS_A500KL_PROJECT)
	int Vf900TableSec[20] = {179,183,188,183,179,179,181,179,180,193,191,185,190,187,198,198,141,132,82, 47};
#else
...
static void cal_bat_capacity_work(struct work_struct *work)
{
	...
	printk("[BAT][Gau] enter cal_bat_capacity_work!\n");
...
int AXC_Gauge_A66_AskCapacity(struct AXI_Gauge *gauge)
{
	...
	printk("[BAT][Gau] enter AXC_Gauge_A66_AskCapacity!\n");
	BUG_ON(NULL == gauge);
...
int AXC_Gauge_A66_GetNextPollingInterval(struct AXI_Gauge *gauge)
{
	...
	if (ReportCapToGaugeForJudgeInterval() < OCV_PER_SPEEDUP_UPDATE_35P)
	...
	int *curr)
{
	...
	BUG_ON(NULL == volt);
	BUG_ON(NULL == curr);
	printk("[BAT][Gau] enter AXC_Gauge_A66_ReadVoltCurrWithoutCali!\n");
	while (i < anTimes)
	{
...
{
	AXC_Gauge_A66 *this = container_of(apGauge, AXC_Gauge_A66, msParentGauge);
	printk("[BAT][Gau] enter AXC_Gauge_A66_Constructor!\n");
	BUG_ON((NULL == gaugeCb) || (NULL == apGauge));

//ASUS_BSP Eason read battery ID+++
#if defined(ASUS_A600KL_PROJECT)
	CurrOCVtoPerTbl = A600KL_SrcOCVtoPerTbl;
#elif defined(ASUS_A500KL_PROJECT)
	CurrOCVtoPerTbl = A500KL_SrcOCVtoPerTbl;
#else
	if(g_BatteryID_value<1100000){
