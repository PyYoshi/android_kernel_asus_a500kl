...
#include <linux/i2c_testcase.h>//ASUS_BSP +++ pansy_chen "[A500KL][Sensor] Ak09911 E-compass I2C Stress Test"
...
int ecompass_status = 0;	//ASUS_BSP + 
...
AKECS_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ECS_IOCTL_GET_DELAY:
		...
		break;
	case ECS_IOCTL_GET_LAYOUT:
//ASUS_BSP +++ pansy_chen "[A500KL][Sensor][NA][Spec] add compass layout parameter"
       akm->layout = PAT2;
//ASUS_BSP +++ pansy_chen "[A500KL][Sensor][NA][Spec] add compass layout parameter"
...
//ASUS_BSP +++ Jason Chang "[A20][Sensor][NA][ATD]ATD interface for 9-axis sensor"
static ssize_t read_ecompass_status(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	printk("read_ecompass_status = %d",ecompass_status);
	return sprintf(buf, "%d\n", ecompass_status);
}
//ASUS_BSP ---
//ASUS_BSP +++ pansy_chen "[A500KL][Sensor][NA][Spec] Porting Ak09911 E-compass"
static ssize_t akm_sysfs_set_mode(
	struct device *dev, struct device_attribute *attr,
	char const *buf, size_t count)
{
	int ret=0;
	...
	switch(mode){
		case 0://Power-down mode
		   regs[1] = 0x00;
		   break;
		case 1://Single measurement mode
		   regs[1] = 0x01;
		   break;
		case 2://Continuous measurement mode 1
		   regs[1] = 0x02;
		   break;
		case 3://Continuous measurement mode 2
		   regs[1] = 0x04;   
		   break;
	    case 4://Continuous measurement mode 3
		   regs[1] = 0x06;   
		   break;
		case 5://Continuous measurement mode 4
		   regs[1] = 0x08;   
		   break;
		case 6://Self-test mode
		   regs[1] = 0x10;   
		   break;
		case 7://Fuse ROM access mode
		   regs[1] = 0x1F;   
		   break;   
		default:
		      printk("unkown mode!\n");
		...
		ret = akm_i2c_txdata(akm->i2c, regs, 2);
		return 1;
}
//ASUS_BSP--- pansy_chen "[A500KL][Sensor][NA][Spec] Porting Ak09911 E-compass"
//ASUS_BSP +++ pansy_chen "[A500KL][Sensor][NA][Spec] Add ATD Interface of Ak09911 E-Compass"
static ssize_t akm_sysfs_get_raw(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	uint8_t buffer[AKM_SENSOR_DATA_SIZE];
	int err;
	...
	/* Read status */
	buffer[0] = AKM_REG_STATUS;
	err = akm_i2c_rxdata(akm->i2c, &(buffer[0]), AKM_SENSOR_DATA_SIZE);
	if (err < 0) {
		dev_err(&akm->i2c->dev, "%s failed.", __func__);
		return err;
	}
    temp[0]=(buffer[2]<<8)|buffer[1];
    temp[1]=(buffer[4]<<8)|buffer[3];
    temp[2]=(buffer[6]<<8)|buffer[5];
    return scnprintf(buf,PAGE_SIZE,"Mag raw:mx:%d, my:%d, mz:%d\n",temp[0],temp[1],temp[2]);
}
static ssize_t akm_sysfs_op_mode(
	struct device *dev, struct device_attribute *attr,
	char const *buf, size_t count)
{
	int ret=0;
...
static struct device_attribute akm_compass_attributes[] = {
   ...
   __ATTR(compass_status, 0664, read_ecompass_status, NULL),
   //ASUS_BSP +++ pansy_chen "[A500KL][Sensor][NA][Spec] Porting Ak09911 E-compass"
   __ATTR(set_mode, 0664, NULL, akm_sysfs_set_mode),
   //ASUS_BSP --- pansy_chen "[A500KL][Sensor][NA][Spec] Porting Ak09911 E-compass"
   //ASUS_BSP +++ pansy_chen "[A500KL][Sensor][NA][Spec] Add ATD Interface of Ak09911 E-Compass"
   __ATTR(get_raw, 0664, akm_sysfs_get_raw, NULL),
   //ASUS_BSP --- pansy_chen "[A500KL][Sensor][NA][Spec] Add ATD Interface of Ak09911 E-Compass"
   //ASUS_BSP +++ pansy_chen "[A500KL][Sensor][NA][ATD] Add ATD Interface of Ak09911 E-Compass"
   __ATTR(op_mode, 0664, NULL, akm_sysfs_op_mode),
   //ASUS_BSP --- pansy_chen "[A500KL][Sensor][NA][STD] Add ATD Interface of Ak09911 E-Compass"
#endif
	__ATTR_NULL,
};
...
//ASUS +++ 
int akm_set_regulator(struct i2c_client *client, int vol_min, int vol_max){

	int ret=0;
	static struct regulator *ldo;

	ldo = regulator_get(&client->dev, "vdd_ana");
	if (IS_ERR(ldo)) {
		dev_err(&client->dev,"%s: Failed to get vdd_ana regulator\n", __func__ );
		ldo = NULL;
		return ret;
	}
	else
		printk("%s: get reg success\n", __func__);

	if (regulator_set_voltage(ldo, vol_min, vol_max)) {
		dev_err(&client->dev,"%s: VREG vdd_ana set voltage failed\n",  __func__ );
		goto ldo_disable;
	}
	else
		dev_err(&client->dev,"%s set voltage success\n", __func__);
	
	if (regulator_enable(ldo)) {
		dev_err(&client->dev,"%s: VREG vdd_ana enable failed\n", __func__);
		goto ldo_put;
	}
	else
		dev_err(&client->dev,"%s: vdd_ana enable reg\n", __func__);
...
//ASUS ---
//ASUS_BSP +++ pansy_chen "[A500KL][Sensor] Ak09911 E-compass I2C Stress Test"
#ifdef CONFIG_I2C_STRESS_TEST
static int TestAKM09911SensorI2C (struct i2c_client *apClient)
{
	int ret=0;
...
//	i2c_log_in_test_case("%s +\n",__func__);
	...
	if(s_akm != NULL)
	ret = akm_compass_sysfs_enable_store(s_akm, "1", 1, MAG_DATA_FLAG);
	if(ret<0){
		lnResult = -1;
		i2c_log_in_test_case("%s enable compass failed\n",__func__);
		goto exit;
	}
	msleep(1000);

	ret = akm_compass_sysfs_enable_store(s_akm, "0", 1, MAG_DATA_FLAG);
	if(ret<0){
		lnResult = -1;
		i2c_log_in_test_case("%s disable compass failed\n",__func__);
		goto exit;
	}
//	i2c_log_in_test_case("%s -\n",__func__);
exit:
...
static struct i2c_test_case_info gMSensorTestCaseInfo[] =
{
     __I2C_STRESS_TEST_CASE_ATTR(TestAKM09911SensorI2C),
};
#endif
//ASUS_BSP --- pansy_chen "[A500KL][Sensor] Ak09911 E-compass I2C Stress Test"
...
//ASUS_BSP +++ pansy_chen "[A500KL][Sensor][NA][Spec] Porting Ak09911 E-compass"
    printk("pansy:akm_compass_probe++\n");	
	//strcpy(ldo_name, "8226_l19");
	vol_min = 2850000;
	vol_max = 2850000;
    akm_set_regulator(client,vol_min,vol_max);
//ASUS_BSP --- pansy_chen "[A500KL][Sensor][NA][Spec] Porting Ak09911 E-compass"
	...
	if (0 > err) {
		printk("%s: create sysfs failed.", __func__);
		goto exit6;
	}
   ecompass_status = 1;	//ASUS_BSP +

//ASUS_BSP +++ pansy_chen "[A500KL][Sensor] Ak09911 E-compass I2C Stress Test"
#ifdef CONFIG_I2C_STRESS_TEST
	i2c_add_test_case(client, "MagSensorTest",ARRAY_AND_SIZE(gMSensorTestCaseInfo));
#endif
//ASUS_BSP --- pansy_chen "[A500KL][Sensor] Ak09911 E-compass I2C Stress Test"
